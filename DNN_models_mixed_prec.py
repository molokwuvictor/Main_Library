...
def plain_modular_model(cfd_type=None,inputs=None,width={'ps':{'Bottom_Size':None,'No_Gens':None,'Growth_Type':None,'Growth_Rate':None},'ko':{},'Bu':{}},depth={'ps':None,'ko':None,'Bu':None},\
    ...
    #var_dtype=inputs.dtype_policy.variable_dtype
    var_dtype=policy.variable_dtype

    def model_wrapper (cfd_type,train_stats): 
        model_wrapper.cfd_type=cfd_type
        model_wrapper.ts=tf.Variable(tf.cast(train_stats[0],dtype=var_dtype),trainable=False)
    ...
    t_n=split_Layer(no_splits=aux_labels.shape[-1],axis=-1,layer_name='time_n',reshape=aux_labels.shape[:-1],dtype=var_dtype)(aux_labels)[0]
    ...
    pre_hardlayer=output_Hardlayer(norm_limits=[cfd_type['Norm_Limits'],cfd_type['Norm_Limits']],init_value=cfd_type['Pi'],co_var='no_correlate',input_activation=hardlayer_act_func,layer_name='pressure_hardlayer',\
                                   kernel_exponent=[{'Value':0.57721*(1/3),'Trainable':True,'Lower_Limit':0.,'Upper_Limit':0.57721,'Rate':1.},{'Value':1.,'Trainable':False,'Lower_Limit':0.1,'Upper_Limit':10,'Rate':1.}],regularization=0.00,weights=[],model=model,cfd_type=cfd_type,dtype=tf.keras.mixed_precision.global_policy().variable_dtype)
    ...
    if cfd_type['Type'].upper()=='PINN':
        p_int=pre_hardlayer([[inputs_list[-2],inputs_list[-1]],p_int],time_n0=t_n)
    ...
    if batch_norm['Pre_Rect']['Use_Pre_Rect']:
        ...
        p_int=identity_Layer(layer_name='pre_rect_layer',dtype=var_dtype)(p_int)
    ...
    p=identity_Layer(layer_name='identity_layer_pressure',dtype=var_dtype)(p_int)
    ...
    s_hlayer=layers_Gas_Oil_Saturation(inputs=inputs,pre=p_int,config=config,cfd_type=cfd_type,width=width,depth=depth,hlayer_activation_func=hlayer_activation_func,\
                                           olayer_activation_func=olayer_activation_func,kernel=kernel,residual_module_par=residual_module_par,layer_name='gsat_osat_layer',solu_type='')
    ...
    if cfd_type['Type'].upper()=='PINN':
        ...
        tf.keras.mixed_precision.set_global_policy(policy.variable_dtype)
    ...
    if 'Truncation_Error_Model' in cfd_type and cfd_type['Type'].upper()=='PINN':
        ...
        # residual_module_par_dt['Kernel_Init']='lecun_normal'
        # residual_module_par_dt['Latent_Layer']['Width']*=1.0
        # dt_out=dnn_sub_block(inputs=inputs,width=width['tr'],depth=depth['tr'][0],activation_func=hlayer_activation_func['tr'],kernel_init=kernel['Init'],kernel_regu=kernel['Regu']['tr'],name='time_split',olayer_act_func=olayer_activation_func['dt'],residual_module_par=residual_module_par_dt)
        ...
        # dt_out=output_Hardlayer(norm_limits=[cfd_type['Norm_Limits'],[5000.,14.7]],co_var='correlate',init_value=0.002,input_activation=None,layer_name='time_hardlayer',\
        # kernel_activation=[lambda x:-x,None],regularization=0.00,weights=[],model=model,cfd_type=cfd_type)([[inputs_list[-2],p_int],dt_out])
        # dt_out=timestep_Hardlayer(layer_name='time_hardlayer',alpha={'Value':0.1,'Trainable':False,'Lower_Limit':0.,'Upper_Limit':0.99,'Rate':1.},tshift=5.,model=model,cfd_type=cfd_type,activation_func=olayer_activation_func['dt'])([t_n,inputs_list[-2]],dt_out)
        q_well_idx=expand_dims_Layer(conn_idx=cfd_type['Conn_Idx'],qoi=tf.ones_like(cfd_type['Init_Grate']),dim=cfd_type['Dimension']['Dim'],layer_name='expand_dims_q_well_idx',)(trn_out)
    ...
    if cfd_type['Type'].upper()=='PINN':
         ...
         if cfd_type['Compute_Rate_BHP']:
             ...
             if cfd_type['Fluid_Type'] in ['GC','gas_cond']:
                 inputs_rate_bhp.update({'InvBo':invBo,'Invuo':invuo,'Rs':Rs,'Rv':Rv,'So':s[1]})
             opt_rate,BHP=computeRateBHP(model=model,config=config,poly_coeff_Bu=poly_coeff_Bu,PVT_layer=PVT_layer,shape=dt_out.get_shape()[0],dtype=var_dtype)(inputs_rate_bhp)
...
class computeRateBHP(tf.keras.layers.Layer):
    def __init__(self,C=0.001127,D=5.6145833334,model=None,config=None,poly_coeff_Bu=None,scal=None,PVT_layer=None,cname='Compute_Rate_BHP_Layer',shape=[None,39,39,1],**kwargs):
    ...
    def call(self, x):
        ...
        if self.model.cfd_type['Fluid_Type'] in ['GC','gas_cond']:
            ...
            q_ij_opt,pwf=Sg_ij,Sg_ij
        else:
            q_ij_opt,pwf=batch_loss.compute_rate_bhp_gas(p_ij,invBgug_ij,krg_ij,q_t0_ij,min_bhp_ij,Ck_ij,_shutins_idx=shutins_idx,_invBg_n1_ij=invBg_ij,_no_intervals=2,_ctrl_mode=1,_q_well_idx=q_well_idx,model_PVT=self.PVT_layer)
...
def layers_PVT(inputs=None, config=None,cfd_type=None,poly_coeff_Bu=None,name='',):
    ...
    if config['Bu'][0] in ['plain','res','cnn2d']:
        ...
        if cfd_type['Fluid_Type'] in ['gas_cond','GC']:
            ...
            if name_Bo.upper()=='POLY':
                ...
                invBo=restep_Layer(limits=[0.,cfd_type['Dew_Point']],values=[0,cfd_type['Init_InvBo']],layer_name='InvBo_Limit_Dew_Point')([pre_input,spline_Interpolation(points_x=poly_coeff_Bu['Pre'],values_y=poly_coeff_Bu['InvBo'],order=order_Rs,layer_name='InvBo_spline'+name)(pre_input)])
    ...
    # Return outputs; reset global policy
    out_PVT=[invBg,invug]
    if cfd_type['Fluid_Type'] in ['gas_cond','GC']:
...
def dnn_sub_block(inputs=None,width={},depth=None,activation_func=None,olayer_act_func=None,kernel_init='glorot_normal',kernel_regu=None, name=None,residual_module_par={},layer_LSTM={'Add':False},layer_CNN={'Add':False}):
    ...
    if depth<=0:
        ...
        if residual_module_par['Network_Type']=='RESN':
            hlayers_gsat=dnn1.RNN_Layer(depth=depth,width=width,res_par=residual_module_par,layer_name=name+'gsaturation',gaussian_process=None,out_act_func=olayer_act_func,idx=nstack_idx,batch_norm={'Use_Batch_Norm':False,},dropout={'Add':False,},attn={})
            hlayer=hlayers_gsat(inputs,output_layer=True)
    ...
    def __init__(self,ts,cfd_type,*args, **kwargs):
        ...
        if self.cfd_type['Type']=='PINN':
            ...
            if self.cfd_type['Fluid_Type'] in ['dry-gas','DG']:
                self.loss_func['Physics_Error']=batch_loss.physics_error_gas_2D
                ...
                self.loss_func['Physics_Error']=batch_loss.physics_error_gas_oil_2D
            ...
            self.loss_func['Batch_Loss']=batch_loss.pinn_batch_sse_grad    
            self.loss_func['Physics_Error']=batch_loss.zeros_like_pinn_error
    ...
    def train_step(self, data):
        ...
        if self.cfd_type['Accum_Grad']['Add']==True:
            ...
            if tf.keras.mixed_precision.global_policy().compute_dtype=='float16':
                batch_wmse_grad=self.optimizer.get_unscaled_gradients(batch_wmse_grad)
...
class identity_Layer(tf.keras.layers.Layer):
    def __init__(self,constant=1.,trainable=False,layer_name='',**kwargs):
        super(identity_Layer, self).__init__(**kwargs,name=layer_name)
        self.constant=tf.Variable(constant,trainable=trainable,dtype=self.dtype)
    def call(self, inputs):
        return self.constant*(inputs)  

class constant_Layer(tf.keras.layers.Layer):
    def __init__(self,constant=1.,trainable=False,layer_name='',mean=0.,stddev=0.,**kwargs):
        super(constant_Layer, self).__init__(**kwargs,name=layer_name)
        self.constant=tf.Variable(constant,trainable=trainable,dtype=self.dtype)
        self.mean=mean
        self.stddev=stddev
    def call(self, inputs):
...
class constantDiff_Layer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        return self.constant*tf.ones_like(inputs)-inputs 
...
class expand_dims_Layer(tf.keras.layers.Layer):
    def __init__(self,conn_idx=None,qoi=None,dim=None,layer_name='',**kwargs):
        super(expand_dims_Layer, self).__init__(**kwargs,name=layer_name)
        self.conn_idx=conn_idx
        self.qoi=qoi
        self.dim=dim
    def call(self, inputs):
        return tf.cast(tf.expand_dims(tf.scatter_nd(self.conn_idx, self.qoi, self.dim),0),inputs.dtype)*tf.ones_like(inputs)    
...
class split_Layer(tf.keras.layers.Layer):
    def __init__(self,no_splits=None,axis=-1,layer_name='',reshape=None,**kwargs):
        super(split_Layer, self).__init__(**kwargs,name=layer_name)
...
class binary_Layer(tf.keras.layers.Layer):
    def __init__(self,limits=[0.,],values=[None,1.],layer_name=''):
    ...
    def call(self, inputs):
        ...
        out=(tf.cast(c<=self.limits[0],self.dtype)*self.values[0](c))+\
            (tf.cast(c>self.limits[0],self.dtype)*self.values[1](c))
        return out
...
class scaled_relu_Layer(tf.keras.layers.Layer):
    def __init__(self,limits=[0,1],act_func=[tf.nn.sigmoid],layer_name='',alpha=1.,trainable=False,lower_limit=0.01,upper_limit=0.99,dew_point=4048.4854,beta=0.):
    ...
    def call(self, inputs):
        out=(tf.cast(inputs<self.limits[0],self.dtype)*self.limits[0])+(tf.cast((inputs>=self.limits[0])&(inputs<=self.limits[1]),self.dtype)*inputs)+\
             (tf.cast(inputs>self.limits[1],self.dtype)*(self.limits[1]+self.beta*inputs))

        out=self.alpha*self.act_func[0](out)
        #out=(tf.cast(pre>=self.pdew,self.dtype)*0.)+(tf.cast((pre<self.pdew),self.dtype)*self.act_func[0](out)*self.alpha) 
        return out

class scaled_lishtplus_Layer(tf.keras.layers.Layer):    
    def __init__(self,layer_name='',alpha=1.,trainable=False,lower_limit=1.,upper_limit=10.,):
        super(scaled_lishtplus_Layer, self).__init__(name=layer_name)
        self.trainable_kernel=trainable
        self.init_value=tf.constant_initializer(value=alpha)
        self.alpha=tf.Variable(initial_value=self.init_value([1]),dtype=self.dtype,constraint=tf.keras.constraints.MinMaxNorm(min_value=lower_limit,\
                            max_value=upper_limit, rate=1., axis=0),trainable=trainable)
...
class gaussian_blur_Layer(tf.keras.layers.Layer):    
    def __init__(self,layer_name='',sigma=0.1,trainable=False,lower_limit=0.,upper_limit=0.5,model=None,f_idx=5,mean=3.):
...
class timestep_Hardlayer(tf.keras.layers.Layer):    
    def __init__(self,layer_name='',norm_limits=[[-1,1],[-1,1]],init_value=0.,alpha={'Value':0.1,'Trainable':False,'Lower_Limit':0.1,'Upper_Limit':1.,'Rate':1.},\
                 tshift=10.,model=None,cfd_type=None,activation_func=tf.nn.softplus):
    ...
    def call(self,tn,inputs):
        tstep=inputs#tf.broadcast_to(self.activation_func(tf.reduce_mean(inputs,axis=[1,2,3],keepdims=True),tf.shape(inputs))#tf.ones_like(inputs))
        t0=bl.nonormalize(self.model,tn[0],stat_idx=3,compute=True)
        t1=bl.nonormalize(self.model,tn[1],stat_idx=3,compute=True)
        #alpha=(tnorm-self.norm_limits[0][0])/(self.norm_limits[0][1]-self.norm_limits[0][0])
        #tstep=tstep*self.alpha
        welopens_idx=tf.reduce_mean([tf.reduce_mean([self.wel_opens(limits=self.cfd_type['Connection_Shutins']['Days'][c][cidx],time_n0=t0,time_n1=t1,dtype=self.dtype,tshift=self.tshift) for cidx in self.cfd_type['Connection_Shutins']['Shutins_Per_Conn_Idx'][c]],axis=0) for c in self.cfd_type['Connection_Shutins']['Shutins_Idx']],axis=0)
        tstep=(tf.cast(welopens_idx>0.,self.dtype)*tstep*self.alpha)+(tf.cast(welopens_idx<=0.,self.dtype)*tstep)
        
        return tstep
    def wel_opens(self,limits=None,time_n0=None,time_n1=None,dtype=None,tshift=10.):
        return (tf.cast((time_n0==time_n1)&(((time_n0>=(limits[0]-tshift))&(time_n0<=(limits[0]+tshift)))|((time_n0>=(limits[1]-tshift))&(time_n0<=(limits[1]+tshift))))&(limits[1]>0.),dtype)) 
...
class normalization_Layer(tf.keras.layers.Layer):
    def __init__(self,stat_limits=[14.7,5000.],norm_limits=[-1,1],norm_type='linear',layer_name=''):
...
class spline_Interpolation(tf.keras.layers.Layer):
    def __init__(self,points_x=None,values_y=None,order=1,layer_name='',):
        ...
        self.order=order
    def call(self, inputs):
        return tfa.image.interpolate_spline(tf.cast(self.points_x,inputs.dtype),tf.cast(self.values_y,inputs.dtype),inputs,self.order)
    
class output_Hardlayer(tf.keras.layers.Layer):
    def __init__(self,norm_limits=[[-1,1],[-1,1]],co_var='no_correlate',init_value=None,activation_function_inputs=None,kernel_activation=[None,None],input_activation=None,layer_name='',\
        kernel_exponent=[{'Value':1.,'Trainable':False,'Lower_Limit':0.,'Upper_Limit':0.99,'Rate':1.},{'Value':1.,'Trainable':False,'Lower_Limit':0.,'Upper_Limit':0.99,'Rate':1.}],regularization=0.,weights=[0.],model=None,cfd_type=None,**kwargs):
        ...
        # self.depth_dense=1
        self.rbf_dim=50
        #self.dense_layer=(tf.keras.layers.Dense(32, activation=tf.nn.swish,**self._get_common_kwargs_for_layer()))
        self.dense_output_k=tfa.layers.SpectralNormalization(tf.keras.layers.Dense(1, activation=tf.nn.swish,dtype=self.dtype,**self._get_common_kwargs_for_layer()),dtype=self.dtype)  #lambda x:1.+tf.math.exp(-x)
        ...
        # self.mha=tf.keras.layers.MultiHeadAttention(key_dim=1,num_heads=2,**kwargs) 

    def call(self, inputs,time_n0=-1.):
        tn1=inputs[0][0]
        tn2=inputs[0][1]
        p=inputs[1]

        t1=bl.nonormalize(self.model,(tn1),stat_idx=3,compute=True)
        t0=bl.nonormalize(self.model,(time_n0),stat_idx=3,compute=True)
        treg_fac=tf.reduce_mean([tf.reduce_mean([self.shut_days(limits=self.cfd_type['Connection_Shutins']['Days'][c][cidx],time_n0=t0,time_n1=t1,dtype=self.dtype,rate=.5) for cidx in self.cfd_type['Connection_Shutins']['Shutins_Per_Conn_Idx'][c]],axis=0) for c in self.cfd_type['Connection_Shutins']['Shutins_Idx']],axis=0)
        
        # if init_layer is not None:
        #     self.init_value=init_layer
        beta=((tn2-self.norm_limits[1][0])/(self.norm_limits[1][1]-self.norm_limits[1][0]))
        lmd=(tf.cast(beta<=0.,tn2.dtype)*tf.zeros_like(beta))+(tf.cast(beta>0.,tn2.dtype)*tf.ones_like(beta))
        #lmd=(tf.cast(beta<=1.,tn2.dtype)*tf.zeros_like(beta))+(tf.cast(beta>1.,tn2.dtype)*tf.ones_like(beta))
        ...
        # Permeability
        alpha_k=tf.reshape(self.rbf_k(tf.reshape(tn2,(-1,1))),tf.concat([tf.shape(p)[0:-1],[tf.shape(p)[-1]*self.rbf_dim]],0))
        ...
        alpha=alpha_k*(alpha)**(self.kernel_exponent[0][0])
...
class PVT_Model(tf.keras.layers.Layer):
    def __init__(self, config=None,cfd_type=None,poly_coeff_Bu=None,list_idx=[0,1],layer_name='',**kwargs):
        super(PVT_Model, self).__init__(**kwargs,name=layer_name)
        self.config=config
        self.cfd_type=cfd_type
        self.poly_coeff_Bu=poly_coeff_Bu
...
def layers_Gas_Oil_Saturation(inputs=None,pre=None,config=None,cfd_type=None,width=None,depth=None,hlayer_activation_func=None,olayer_activation_func=None,kernel=None,residual_module_par=None,layer_name='',solu_type=''):
    var_dtype=tf.keras.mixed_precision.global_policy().variable_dtype
    residual_module_par_ps=residual_module_par
    # if solu_type.upper()=='PRESSURE_DEPENDENT':
    #     inputs_pre=normalization_Layer(stat_limits=[14.7,cfd_type['Pi']],norm_limits=cfd_type['Norm_Limits'],layer_name='pre_normalize')(pre)
    Sgi=1-cfd_type['SCAL']['End_Points']['Swmin']
    ...
    if config['ps'][2] is not None:
        ...
        width['ps']['Growth_Rate']=2.0
        if cfd_type['Type'].upper()=='PINN' and config['ps'][2]=='cnn2d':
        ...
        if cfd_type['Type'].upper()=='PINN':
            #so=restep_Layer(limits=[cfd_type['Dew_Point']],layer_name='pre_oil_saturation_comp_restep_layer')([pre,sg])
            pdew=cfd_type['Dew_Point']
            pi=cfd_type['Pi']
            ...
            binary_func=binary_Layer(limits=[0.,],values=[0.,tf.nn.relu],layer_name='binary_activation_layer')
            ...
            hardlayer_act_func=tf.nn.sigmoid
            ...
            sg=output_Hardlayer(norm_limits=[[-1,1],[pdew,14.7]],co_var='correlate',init_value=Sgi,kernel_activation=[None,tf.nn.relu],input_activation=hardlayer_act_func,layer_name='gsaturation_hardlayer',regularization=0.00,weights=[],kernel_exponent=[{'Value':0.5773/3,'Trainable':True,'Lower_Limit':0.1,'Upper_Limit':1.,'Rate':1.},{'Value':1.,'Trainable':False,'Lower_Limit':0.1,'Upper_Limit':1.,'Rate':1.}],model=model,cfd_type=cfd_type)([[inputs_list[-2],pre],sg])
            #sg=output_Hardlayer(norm_limits=[[-1,1],[pdew,14.7]],co_var='no_correlate',init_value=Sgi,kernel_activation=[None,None],input_activation=hardlayer_act_func,layer_name='gsaturation_hardlayer',regularization=0.00,weights=[],kernel_exponent=[{'Value':1.5,'Trainable':True,'Lower_Limit':1.,'Upper_Limit':2.,'Rate':1.},{'Value':0.5,'Trainable':False,'Lower_Limit':0.1,'Upper_Limit':1.,'Rate':1.}],model=model,cfd_type=cfd_type)([[inputs_list[-2],pre],sg])
            #sg=output_Hardlayer(norm_limits=[[pdew,14.7],[-1,1]],co_var='no_correlate',init_value=Sgi,kernel_activation=[tf.nn.relu,None],input_activation=hardlayer_act_func,layer_name='gsaturation_hardlayer',regularization=0.00,weights=[],kernel_exponent=[{'Value':0.5773/3,'Trainable':True,'Lower_Limit':0.1,'Upper_Limit':1.,'Rate':1.},{'Value':0.5773/3,'Trainable':False,'Lower_Limit':0.1,'Upper_Limit':1.,'Rate':1.}],model=model,cfd_type=cfd_type)([[pre,inputs_list[-2]],sg])
            #sg=multiply_Layer(layer_name='gas_saturation_hardlayer_scaled')(sg,y=1.,constraint=[(Sgi-0.3),Sgi])
    else:
        #sg=constant_Layer(constant=Sgi,layer_name='unmodelled_layer_gsaturation',mean=0.,stddev=0.)(inputs)
        sg=constant_Layer(constant=Sgi,layer_name='unmodelled_layer_gsaturation',mean=0.,stddev=0.,dtype=var_dtype)(inputs_list[-2])
    out_sat=[sg]
    ...
    if cfd_type['Fluid_Type'] in ['gas_cond','GC']:
        if config['ps'][3] is not None:
            ...
            so=constantDiff_Layer(constant=(1-cfd_type['SCAL']['End_Points']['Swmin']),layer_name='unmodelled_layer_osaturation',dtype=var_dtype)(sg) 
        out_sat+=[so]
    return out_sat
...
def relative_permeability(endpoints={'kro_Somax':0.90,'krg_Sorg':0.80,'krg_Swmin':0.90,'Swmin':0.22,'Sorg':0.2,'Sgc':0.05},corey_exp={'nog':3.,'ng':6.,'nw':2.},dtype=tf.float32):
